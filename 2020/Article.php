...
class Article_Controller extends Base_Controller {
	...
	public function index() {	
		...
		if (Wee::$config['html_cache_on']) {
			$htmlFile = "article/" . floor($this->id / 100) . "/{$this->id}_{$this->page}.dwt";
	...
	public function makeHtml() {
		...
		$this->page = $this->input->getIntval('p');
		$this->_show();
		$this->output->makeHtml($this->template, load_model('Article')->getPath($this->id, $this->page));
	}
	//图图系统
	public function _show() {
		...
		if (1 == $this->cate['view_type']) {
			Wee::$config['web_article_pagenum'] = Wee::$config['web_article_pagenum'];
			$article['thumb'] = $modAttach->getThumbList($this->id, null);
			$this->template = 'slide.dwt'; 
		} else {
			Wee::$config['web_article_pagenum'] = Wee::$config['web_article_pagenum'];
			$article['thumb'] = $modAttach->getThumbList($this->id, null);
			$this->template = 'article.dwt';	
		}
		$article['pre'] = $modArticle->getPre($this->id,$article['cid']);
		$article['next'] = $modArticle->getNext($this->id,$article['cid']);
		$url = array($modArticle, 'getUrl', $this->id);
		$article['attach_num'] = $modAttach->getAttachNum($this->id);
		$pageInfo = new Ext_Page($url, $article['attach_num'], $this->page, Wee::$config['web_article_pagenum'], 10);	
		$article['total_page'] = $pageInfo->totalPage();		
		$article['attach'] = $modAttach->getAttachList($this->id, $pageInfo->limit());
		$article['pre_url'] = $modArticle->getUrl($this->id, $this->page - 1);
		if ($this->page < $article['total_page']) {
			$article['next_url'] = $modArticle->getUrl($this->id, $this->page + 1);	
		} elseif ($article['next']) {
