...
class Install_Controller extends Controller {
	public function __construct() {
		...
		if (is_file($this->lockFile)) {
			show_msg('系统已经安装过程序请不要重复安装,如需重新安装请删除data目录下install文件夹后再次安装', null, -1);	
		}
	...
	public function setup4() {
		...
		foreach ($data as $name => $value) {
			$this->db->table('#@_config')
				->replace(array('name' => $name, 'value' => $value));	
		}
...
if (is_file(Wee::\$config['data_path'] . 'web-config.php')) {
	...
	private function _isWritable($dir) {
		$dir = trim($dir, "\\/");
		if ($dir) {
			$testfile = APP_PATH . "$dir/tutucms.test";
		} else {
			$testfile = APP_PATH . "tutucms.test";	
		}
